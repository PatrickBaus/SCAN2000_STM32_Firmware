    ...
    *relayUnsetRegister = 0x0000;
    ...
    if (command != 0x00000000) {
        ...
        for (uint8_t i = 0; i < 11; i++) {
            ...
            *relaySetRegister |= (uint32_t)!!(command & (1UL << scan2000ChannelOnSequence[i])) << (i + (i / 5) * 5);
            // The list of channels, that are to tbe turned off
            *relayUnsetRegister |= (uint32_t)!!(command & (1UL << scan2000ChannelOffSequence[i])) << (i + (i / 5) * 5);
        }
        rv = decodeOK;
    } else {
        rv = decodeIgnored;
    ...
    *relayUnsetRegister = 0x0000;
    ...
    if (command != 0x00000000) {
        ...
        for (size_t i = 0; i < sizeof(scan2000_20ChannelSequence)/sizeof(scan2000_20ChannelSequence[0]); i++) {
            *relayUnsetRegister |= (uint32_t)!!(command & (1ULL << (2 * (scan2000_20ChannelSequence[i] - 1)))) << i;    // Even bits -> turn relays off
            *relaySetRegister |= (uint32_t)!!(command & (1ULL << (2 * (scan2000_20ChannelSequence[i] - 1) + 1))) << i;  // Odd bits -> turn relays on
        }
        return decodeOK;
    }
